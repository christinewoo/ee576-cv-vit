...
In this project, we used two different types of GPUs for experiments.
For Cifar-10 dataset, we trained the model on 2 GTX 2080 Ti cards at a lab Linux machine. \\
For Pokemon dataset, we trained the model on a single GTX 3060 card at a personal Windows 11 macine.
...
The application can take in an image and find our the most similar pokemon based 
on the pretrained model we have in hand.
The output of the application would be in the format of the matched pokemon image it found,
following the name of the pokemon.
\begin{figure}[h]
\begin{center}
% \framebox[4.0in]{$\;$}
